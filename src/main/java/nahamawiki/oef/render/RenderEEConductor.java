...
public class RenderEEConductor extends TileEntitySpecialRenderer {
	...
	private final ModelEEConductor model = new ModelEEConductor(0, 0, false);
	private ModelEEConductor powered = new ModelEEConductor(0, 0, true);
	int lastTick;
	...
	public void renderTileEntityAt(TileEntity tileEntity, double x, double y, double z, float scale) {
		...
		if (machine.getCreeper()) {
			...
			if (f1 != lastTick) {
				lastTick = f1;
				powered = new ModelEEConductor(f1, f1, true);
			}
			powered.setConnectingArray(machine.getConnectingArray());
			GL11.glPushMatrix();
			...
			powered.render((Entity) null, 0, 0, 0, 0, 0, 0.03125F);
